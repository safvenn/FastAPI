...
import { FiPackage, FiClock, FiTruck, FiCheckCircle, FiChevronRight, FiMapPin, FiXCircle } from 'react-icons/fi';
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 min-h-[75vh]">
      ...
      <div className="border-b border-white/5 pb-6 mb-10 text-left">
        <span className="text-xs font-black text-brand-accent tracking-widest uppercase">Purchase Ledger</span>
      ...
      {orders.length > 0 ? (
          ...
          {orders.map((order) => {
            ...
            return (
              <motion.div
                ...
                className="ios-glass rounded-[20px] p-5 space-y-5"
              >
                {/* Upper row: Order Meta */}
                <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-3">
                  {/* Left: order number + date */}
                  <div className="space-y-1">
                    <span className="text-[10px] font-black text-brand-accent tracking-widest uppercase bg-brand-accent/10 px-2 py-0.5 rounded-full">
                      ORDER #{order.id}
                    </span>
                    {order.created_at && (
                  ...
                  <div className="flex items-center gap-3 flex-shrink-0">
                    ...
                    {/* Status pill */}
                    <div
                      className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full border text-[11px] font-bold uppercase tracking-wider ${config.color}`}
                    >
                ...
                {address && (
                  <div className="flex items-center gap-2 text-xs text-neutral-400 font-medium">
                    <FiMapPin className="text-brand-accent w-3.5 h-3.5 flex-shrink-0" />
                    <span>
                ...
                {/* Delivery Timeline — hidden for cancelled orders */}
                {!isCancelled && (
                          ...
                          <div key={node.step} className="flex flex-col items-center relative z-10">
                            <div
                              className={`w-8 h-8 rounded-full border-2 flex items-center justify-center font-extrabold text-xs transition-all duration-300 ${
                                isActiveNode
                                  ? 'bg-brand-bg border-brand-accent text-brand-accent shadow-[0_0_10px_rgba(10,132,255,0.4)]'
                            ...
                            <div className="text-center mt-2">
                              <span
                                className={`text-[10px] font-bold block uppercase tracking-wider ${
                                  isActiveNode ? 'text-white' : 'text-neutral-500'
                                }`}
                              >
                                {node.label}
                              </span>
        ...
        /* Empty State */
        <div className="text-center py-20 border border-dashed border-white/10 rounded-[20px]">
