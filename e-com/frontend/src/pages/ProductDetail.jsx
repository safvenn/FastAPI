...
export default function ProductDetail() {
  ...
  if (loading && !product) {
    return (
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 flex flex-col justify-center items-center min-h-[60vh]">
        <div className="w-10 h-10 border-2 border-brand-muted border-t-brand-accent rounded-full animate-spin"></div>
    ...
    return (
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20 text-center">
        ...
        <button
          ...
          className="mt-6 px-6 py-2.5 bg-white text-black text-xs font-bold uppercase rounded-full hover:bg-brand-accent transition"
        >
          Return to Shop
        </button>
      </div>
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
      ...
      {/* Main product card showcase — ios-glass stage */}
      <div className="ios-glass rounded-[28px] p-8 mb-20">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-start">
          ...
          <div className="lg:col-span-5 text-left space-y-6">
            ...
            <div className="space-y-3">
              {/* Brand Category Tag */}
              <span className="inline-block text-[10px] font-black text-brand-accent bg-brand-accent/15 px-3 py-1 rounded-md uppercase tracking-widest">
                {product.brand} Drop
              </span>
              ...
              {/* Price — marketplace "Last Sale" style */}
              <div className="pt-1">
                <p className="text-[10px] font-black tracking-widest uppercase text-brand-muted mb-0.5">
                  Last Sale
                </p>
            ...
            <div className="space-y-3">
              <div className="flex justify-between items-center">
                ...
                <a href="#" className="text-[10px] text-brand-accent hover:underline font-semibold uppercase tracking-widest">
                  Size Guide
                </a>
              </div>
                    ...
                    return (
                      <button
                        ...
                        className={`py-3 text-xs font-extrabold border rounded-xl transition duration-150 cursor-pointer ${
                          isSelected
                            ? 'bg-brand-accent text-black border-brand-accent font-black shadow-[0_0_8px_rgba(10,132,255,0.2)]'
                            : 'bg-white/5 text-white border-white/10 hover:border-brand-accent/50'
                        }`}
                      >
                        {size}
                      </button>
            ...
            <div className="pt-4">
              <button
                ...
                className={`w-full flex items-center justify-center gap-3 min-h-[44px] py-4 rounded-full font-black text-xs tracking-widest uppercase transition-all duration-300 shadow-xl cursor-pointer focus:outline-none focus:ring-2 focus:ring-brand-accent focus:ring-offset-2 focus:ring-offset-brand-bg ${
                  !selectedSize
                    ? 'bg-neutral-800 text-neutral-500 border border-white/5 cursor-not-allowed'
                    : 'bg-brand-accent text-black hover:scale-[1.02] active:scale-[0.98] shadow-brand-accent/10'
                }`}
              >
            ...
            <div className="grid grid-cols-3 gap-4 pt-6 border-t border-white/5 text-[10px] text-neutral-400">
              <div className="flex flex-col items-center text-center gap-1.5 p-2 bg-white/5 rounded-xl">
                <FiShield className="w-4 h-4 text-brand-accent" />
                <span className="font-bold text-white">LEGIT CHECKED</span>
                <span>100% Authentic</span>
              </div>
              <div className="flex flex-col items-center text-center gap-1.5 p-2 bg-white/5 rounded-xl">
                <FiTruck className="w-4 h-4 text-brand-accent" />
                <span className="font-bold text-white">DOUBLE BOXED</span>
                <span>Secure shipping</span>
              </div>
              <div className="flex flex-col items-center text-center gap-1.5 p-2 bg-white/5 rounded-xl">
                <FiRotateCcw className="w-4 h-4 text-brand-accent" />
      ...
      {relatedProducts.length > 0 && (
        <section className="pt-10 border-t border-white/5">
          <div className="text-left mb-8">
            <span className="text-xs font-black text-brand-accent tracking-widest uppercase">Sneaker Matchers</span>
